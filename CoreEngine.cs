...
namespace GameEngine
{
    public class CoreEngine : Game
    {
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            _spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, null, null, _camera.Transform);
        ...
        private void LoadEntityTemplates()
        {
            ...
            foreach (var file in files)
            {
                ...
                try
                {
                    ...
                    {
                        Console.WriteLine($"Некорректный JSON в файле {file}, пропуск.");
                        continue;
                    }
                    var template = JsonSerializer.Deserialize<EntityTemplate>(content);
                    if (template == null)
                    {
                        Console.WriteLine($"Пропущен некорректный файл шаблона сущности: {file}");
                        continue;
                    }
        ...
        private void LoadLevels()
        {
            ...
            foreach (var file in files)
            {
                try
                {
                    ...
                    {
                        Console.WriteLine($"Некорректный JSON в файле уровня {file}, пропуск.");
                        continue;
                    }
                    var level = JsonSerializer.Deserialize<Level>(content);
                    if (level == null || string.IsNullOrEmpty(level.Id))
                    {
                        Console.WriteLine($"Пропущен некорректный файл уровня: {file}");
                        continue;
                    }
